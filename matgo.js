...
const MATGO_NAMES=['소나무','매화','벚꽃','등꽃','창포','모란','싸리','억새','국화','단풍','비','오동'];
...
function matgoCardHtml(id,opts={}){
 ...
 if(back)return `<span class="matgo-card back ${small?'small':''}"><img src="assets/hwatu/back.svg" alt="화투 뒷면"></span>`;
 const m=matgoMonth(id),k=matgoKind(id),alt=`${m}월 ${MATGO_NAMES[m-1]} ${MATGO_KIND_TEXT[k]}`;
 const tag=clickable?'button':'span';return `<${tag} class="matgo-card ${k} ${small?'small':''} ${selected?'selected':''}" ${clickable?`data-matgo-card="${id}"`:''}><img src="assets/hwatu/card-${String(id).padStart(2,'0')}.svg" alt="${alt}">${mark?`<i>${matgoEsc(mark)}</i>`:''}</${tag}>`;
}
...
function openMatgoLobby(message=''){
 ...
 showModal('류현상 맞고',`<div class="matgo-lobby"><button id="matgoBackHub" class="community-back">← 대전 종목 선택</button><section class="matgo-intro"><div><small>RYU HYUNSANG MATGO</small><h3>후라보노와 맞고 한 판</h3><p>48장 화투로 실제 2인 맞고 방식에 가깝게 진행합니다. 점당 금액은 류현상 키우기의 가상 보유금과 연결됩니다.</p></div><span>🎴</span></section>${message?`<div class="online-quiz-notice">${matgoEsc(message)}</div>`:''}<div class="matgo-money"><span>현재 보유금</span><strong>${money.toLocaleString()}원</strong>${carry>1?`<em>이전 판 나가리: 다음 승부 ${carry}배</em>`:''}</div><section class="matgo-rate-grid">${MATGO_RATES.map(r=>`<button data-matgo-rate="${r}" ${money<r*MATGO_MIN_SCORE?'disabled':''}><span>점당</span><b>${r.toLocaleString()}원</b><small>최소 ${MATGO_MIN_SCORE}점 기준 ${(r*MATGO_MIN_SCORE).toLocaleString()}원 필요</small></button>`).join('')}</section><section class="matgo-rule-summary"><b>적용 룰</b><span>총통 · 흔들기 · 폭탄</span><span>뻑 · 자뻑 · 쪽 · 따닥 · 싹쓸이</span><span>홍단 · 청단 · 초단 · 고도리</span><span>피박 · 광박 · 멍따 · 고박</span><span>3고 이상 배수 · 나가리 이월</span><span>국진은 열끗/쌍피 중 유리한 쪽 자동 계산</span></section></div>`,'matgo');
...
function matgoAfterTurn(side){
 ...
 if(!matgoGame.deck.length&&matgoGame.player.hand.length===0&&matgoGame.ai.hand.length===0){return matgoFinish(null,'nagari')}
 matgoGame.turn=side==='player'?'ai':'player';renderMatgo();if(matgoGame.turn==='ai')matgoScheduleAi();
}
function matgoGoStop(side,go){
 ...
 if(!go)return matgoFinish(side,'stop');p.goCount++;p.lastDecisionScore=matgoCurrentScore(side).total;matgoGame.pending=null;matgoGame.log=`${side==='player'?'류현상':'후라보노'}: GO! 현재 ${p.goCount}고.`;matgoGame.turn=side==='player'?'ai':'player';renderMatgo();if(matgoGame.turn==='ai')matgoScheduleAi();
}
...
function renderMatgo(){
 ...
 $('#modalTitle').textContent='류현상 맞고 · 후라보노';$('#modalBody').innerHTML=`<div class="matgo-board">${choiceHtml}${decisionHtml}<header class="matgo-board-top"><div><b>점당 ${g.rate.toLocaleString()}원</b><span>더미 ${g.deck.length}장</span></div><strong class="${g.turn==='player'?'me':''}">${g.log}</strong><button id="matgoForfeit">기권</button></header><section class="matgo-opponent"><div class="matgo-player-info"><span>후라보노</span><strong>${as.total}점</strong><small>${g.turn==='ai'&&!g.pending?'생각 중...':`손패 ${g.ai.hand.length}장`}</small></div><div class="matgo-opponent-hand">${aiBacks}</div>${matgoCapturedHtml('ai')}</section><section class="matgo-center"><div class="matgo-table-area"><div class="matgo-table-cards">${g.table.length?matgoSortCards(g.table).map(id=>matgoCardHtml(id,{mark:g.ppukByMonth[matgoMonth(id)]?'뻑':''})).join(''):'<span class="matgo-empty-table">바닥이 비었습니다</span>'}</div><div class="matgo-deck-stack"><span class="matgo-card back"><img src="assets/hwatu/back.svg" alt="더미"></span><b>${g.deck.length}</b>${g.lastDrawn!=null?`<div class="matgo-last-card"><small>방금 뒤집은 패</small>${matgoCardHtml(g.lastDrawn,{small:true})}</div>`:''}</div></div></section><section class="matgo-me">${matgoCapturedHtml('player')}<div class="matgo-hand-head"><div><span>류현상</span><strong>${ps.total}점</strong></div><small>${canPlay?'낼 패를 선택하세요.':g.turn==='ai'?'후라보노 차례입니다.':'선택을 마쳐 주세요.'}</small></div>${(bombMonths.length||shakeMonths.length)?`<div class="matgo-special-actions">${bombMonths.map(m=>`<button data-matgo-bomb="${m}">💥 ${m}월 폭탄</button>`).join('')}${shakeMonths.map(m=>`<button data-matgo-shake="${m}">〰️ ${m}월 흔들기</button>`).join('')}</div>`:''}<div class="matgo-my-hand">${playerHand}</div></section></div>`;
