...
const actions={
 ...
 store:[
  ...
  ['복권','1장·10장 자동 구매 모두 시간 미소모 · 주 100장 제한','lottery']
 ],
 practice:[
...
function openLottery(){
 let selected=[];
 const draw=()=>{const used=lotteryPurchasedThisWeek(),remain=Math.max(0,100-used),week=lotteryWeekInfo();showModal('복권 번호 선택',`<div class="info-card"><b>1~45 중 숫자 6개를 선택하세요.</b><p>1장 1,000원 · 시간 미소모 · 10장 자동 구매 10,000원 · 시간 미소모</p><p>${state.day+7}일차 추첨 · 이번 주 ${week.start}~${week.end}일차 구매 <strong>${used}/100장</strong></p></div><div id="lotteryNumbers" class="lottery-number-grid">${lotteryNumberButtons(selected)}</div><p id="lotterySelectionText" class="lottery-selection">선택 ${selected.length}/6 · ${selected.length?selected.join(', '):'번호를 골라 주세요.'}</p><div class="result-actions"><button id="lotteryAuto">자동 선택</button><button id="lotteryReset">초기화</button><button id="lotteryBuy" class="primary" ${selected.length===6&&remain>=1?'':'disabled'}>선택 번호 1장 구매</button><button id="lotteryBuyTen" ${remain>=10?'':'disabled'}>10장 자동 구매</button></div><div class="info-card lottery-prize-table"><small>주간 최대 100장. 1장 구매와 10장 자동 구매 모두 시간이 지나지 않습니다.<br>1등 6개 일치 1억원 · 2등 5개+보너스 2천만원 · 3등 5개 200만원 · 4등 4개 5만원 · 5등 3개 5천원</small></div>`);bind()};
...
function buyLotteryTickets(count=1,numbers=[]){
 ...
 const content=count===1?`<div class="lottery-ticket"><small>추첨 예정 ${drawDay}일차 · 시간 미소모</small><h3>${tickets[0].numbers.map(n=>`<span>${n}</span>`).join('')}</h3><p>7일 뒤 자동으로 당첨 결과가 발표됩니다.</p><button id="lotteryPurchaseConfirm" class="primary wide">확인</button></div>`:`<div class="info-card"><b>복권 10장 자동 구매 완료</b><p>추첨 예정 ${drawDay}일차 · 시간 미소모</p><div class="card-list">${tickets.map((t,i)=>`<small>${i+1}. ${t.numbers.join(', ')}</small>`).join('')}</div><p>이번 주 누적 ${used+count}/100장</p><button id="lotteryPurchaseConfirm" class="primary wide">확인</button></div>`;
...
function finalizeBuskingResult(ctx,rhythmResult=null){
 let {band,type,weather,hpCost,success,quality,fans,money,vocalMultiplier,fameMultiplier,fameLv,gearMultiplier=1}=ctx;
 let rhythmLine='';
 ...
 showDialogue('팬들',`${type} · ${weather.label} 버스킹 ${result}. 체력 ${hpCost} 소모, 팬 ${fans}명, ${money.toLocaleString()}원을 얻었다. 보컬 수입 배율 ${vocalMultiplier.toFixed(2)}배 · 인지도 Lv.${fameLv} 수입 배율 ${fameMultiplier.toFixed(2)}배 · 장비 수입 배율 ${gearMultiplier.toFixed(2)}배.${broken}${durabilityNotice}${rhythmLine}\n\n팬 반응: “${fanLine}”${leaveNote?`\n\n${leaveNote}`:''}\n\n${story}`);
...
function busking(band){
 ...
 const ctx={band,type,weather,hpCost,success,quality,fans,money,vocalMultiplier,fameMultiplier,fameLv,gearMultiplier};if(Math.random()<.35){startBuskingRhythmGame(ctx);return}finalizeBuskingResult(ctx)
}
