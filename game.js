...
function drawerDiscoveryNotice(discovery,onDone){const count=drawerPhotoPieceCount(discovery.photo.id);showBlockingNotice(discovery.complete?'사진 완성':'사진 조각 발견',`<div class="drawer-discovery-card"><div class="drawer-discovery-preview" style="background-image:url('${discovery.photo.image}')"></div><div><b>${drawerEscape(discovery.photo.title)}</b><p>${discovery.complete?'아홉 조각을 모두 모아 사진이 완성되었습니다. 추억 앨범에도 기록됩니다.':`낡은 사진 조각을 발견했습니다. 현재 ${count}/9조각입니다.`}</p><small>서랍의 사진 탭에서 확인할 수 있습니다.</small></div></div>`,onDone)}
...
function render(){
 ...
 $('#actionButtons').innerHTML=visibleActions.map(([n,d,k])=>{if(k==='work'||k==='stockWork'){const job=storeJobInfo(k);n=`${n} · ${job.name} ${job.next?`${job.count}/${job.next}`:`${job.count}회`}`;}const cooldownKey=k==='songSurvival'?'songSurvivalLastDay':k==='quizShow'?'quizShowLastDay':null;const cooldownUsed=cooldownKey&&state.day-state.minigames[cooldownKey]<7;const used=(k==='storePromo'&&state.storeDaily.promoDay===state.day)||(k==='customerPractice'&&state.storeDaily.customerDay===state.day)||(k==='flyerPromo'&&state.storeDaily.flyerDay===state.day)||(k==='observe'&&state.storeDaily.observeDay===state.day)||((k==='busking'||k==='bandBusking')&&state.storeDaily.buskingDay===state.day&&state.storeDaily.buskingCount>=2)||cooldownUsed;const label=cooldownUsed?`${n} · ${7-(state.day-state.minigames[cooldownKey])}일 후 재도전`:n;const drawerNew=k==='drawer'&&(state.drawer?.newPhotos||state.drawer?.newDiary);return `<button class="action-card${drawerNew?' has-collection-news':''}" data-action="${k}" aria-label="${label}${drawerNew?' · 새 수집품 있음':''}${used?' · 이용 제한':''}" ${used?'disabled':''}><b>${label}${drawerNew?'<i class="collection-dot" aria-hidden="true"></i>':''}</b></button>`}).join('');$$('[data-action]').forEach(b=>b.onclick=()=>doAction(b.dataset.action));
}
