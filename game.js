...
function effectiveHpCost(n){n=Math.max(0,Math.ceil(Number(n)||0));if(n<=0)return 0;return energizerActive()?Math.max(1,Math.ceil(n*.8)):n}
...
function openWardrobe(){
 ...
 showModal('옷장',`<div class="card-list">${outfits.map(([x,price],i)=>{const owned=state.ownedOutfits.includes(i);if(i===6&&!owned)return '';const special=i===6;return `<div class="info-card ${special?'mystery-outfit-card':''}"><header><b>${x}</b><span>${special?'수상한 상인의 의상':i===0?'기본 의상':price.toLocaleString()+'원'}</span></header>${special?'<p>착용하는 순간 외모가 최대치 100이 됩니다.</p>':''}<button data-outfit="${i}" ${state.outfit===i?'disabled':''}>${state.outfit===i?'착용 중':owned?'갈아입기':'구매하기'}</button></div>`}).join('')}<div class="info-card"><header><b>헤어·스타일 관리</b><span>500,000원</span></header><p>15일마다 한 번 이용할 수 있습니다. 체력 4를 사용하고 외모 +1. 관리 후 30일이 지나면 외모가 1씩 감소합니다.</p><button id="styleCare" ${styleReason?'disabled':''}>${styleReason||'관리받기'}</button></div></div>`);
...
function describeActionResult(before){
 ...
 const extras=[
  ...
  ['dietPill','다이어트 알약',Number(state.items?.dietPill||0)],
...
function actionAvailability(key){
 ...
 switch(key){
  ...
  case 'work': {const nextStreak=state.economy.lastWorkDay===state.day?Math.max(1,state.economy.workStreak):state.economy.lastWorkDay===state.day-1?Math.max(1,state.economy.workStreak+1):1;reason=actionHpShortage(Math.min(36,22+Math.max(0,nextStreak-1)*2));break}
  case 'stockWork': reason=actionHpShortage(12);break;
...
function render(){
 ...
 $('#actionButtons').innerHTML=visibleActions.map(([n,d,k])=>{if(k==='work'||k==='stockWork'){const job=storeJobInfo(k);n=`${n} · ${job.name} ${job.next?`${job.count}/${job.next}`:`${job.count}회`}`;}if(k==='buyBakcas')n=`${n} · ${state.items.bakcasBoughtToday||0}/3개`;if(k==='snack')n=`${n} · ${state.items.snacksBoughtToday||0}/3개`;if(k==='meditate')n=`${n} · ${actionDailyCount('meditationDay','meditationCount')}/2회`;if(k==='meal')n=`${n} · ${state.items.mealsToday||0}/2회 · 요리 15%`;if(k==='walk')n=`${n} · ${actionDailyCount('walkDay','walkCount')}/2회`;const availability=actionAvailability(k),disabled=availability.disabled,label=n,drawerNew=k==='drawer'&&(state.drawer?.newPhotos||state.drawer?.newDiary);return `<button class="action-card${drawerNew?' has-collection-news':''}" data-action="${k}" aria-label="${label}${availability.reason?` · ${availability.reason}`:''}${drawerNew?' · 새 수집품 있음':''}" ${disabled?'disabled':''}><b>${label}${drawerNew?'<i class="collection-dot" aria-hidden="true"></i>':''}</b>${disabled?`<small class="action-disabled-reason">${availability.reason}</small>`:''}</button>`}).join('');$$('[data-action]').forEach(b=>b.onclick=()=>doAction(b.dataset.action));
}
...
function openItemMenu(){
 ...
 const energizerCount=Math.max(0,Number(state.items.energizer)||0),dietCount=Math.max(0,Number(state.items.dietPill)||0);
 ...
 const dietTen=$('#useDietPillTen');if(dietTen)dietTen.onclick=()=>useDietPill(10)
}
