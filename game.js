...
function render(){
 ...
 $('#actionButtons').innerHTML=visibleActions.map(([n,d,k])=>{if(k==='work'||k==='stockWork'){const job=storeJobInfo(k);n=`${n} · ${job.name} ${job.next?`${job.count}/${job.next}`:`${job.count}회`}`;}if(k==='buyBakcas')n=`${n} · ${state.items.bakcasBoughtToday||0}/3개`;if(k==='snack')n=`${n} · ${state.items.snacksBoughtToday||0}/3개`;const cooldownKey=k==='songSurvival'?'songSurvivalLastDay':k==='quizShow'?'quizShowLastDay':null;const cooldownUsed=cooldownKey&&state.day-state.minigames[cooldownKey]<7;const used=(k==='storePromo'&&state.storeDaily.promoDay===state.day)||(k==='customerPractice'&&state.storeDaily.customerDay===state.day)||(k==='flyerPromo'&&state.storeDaily.flyerDay===state.day)||(k==='observe'&&state.storeDaily.observeDay===state.day)||((k==='busking'||k==='bandBusking')&&state.storeDaily.buskingDay===state.day&&state.storeDaily.buskingCount>=2)||(k==='buyBakcas'&&(state.items.bakcasBoughtToday||0)>=3)||(k==='snack'&&(state.items.snacksBoughtToday||0)>=3)||cooldownUsed;const label=cooldownUsed?`${n} · ${7-(state.day-state.minigames[cooldownKey])}일 후 재도전`:n;const drawerNew=k==='drawer'&&(state.drawer?.newPhotos||state.drawer?.newDiary);return `<button class="action-card${drawerNew?' has-collection-news':''}" data-action="${k}" aria-label="${label}${drawerNew?' · 새 수집품 있음':''}${used?' · 이용 제한':''}" ${used?'disabled':''}><b>${label}${drawerNew?'<i class="collection-dot" aria-hidden="true"></i>':''}</b></button>`}).join('');$$('[data-action]').forEach(b=>b.onclick=()=>doAction(b.dataset.action));
}
