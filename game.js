...
const actions={
 home:[
  ...
  ['서랍','사진 퍼즐 5장·일기 50편 수집 · 시간·능력치 변화 없음','drawer'],
...
function rollDrawerPhotoPiece(actionKey){
 const eligible=['work','stockWork','walk','vocal','compose','arrange'];if(!eligible.includes(actionKey)||drawerTotalPhotoPieces()>=45)return null;
...
function renderDrawerPhotos(){state.drawer.lastTab='photos';state.drawer.newPhotos=false;const pieces=drawerTotalPhotoPieces(),complete=drawerCompletedPhotos().length;return `<div class="drawer-summary"><b>사진 조각 ${pieces}/45</b><span>완성 사진 ${complete}/5</span></div><p class="drawer-help">편의점 알바·야간 진열·산책·보컬 연습·작곡 연습·신곡 편곡 중 낮은 확률로 조각을 발견합니다. 12회 연속 실패부터 확률이 오르고, 18회 연속 실패 후에는 다음 대상 행동에서 반드시 발견합니다.</p><div class="drawer-photo-grid">${drawerPhotoCatalog.map(photo=>{const count=drawerPhotoPieceCount(photo.id);return `<button class="drawer-photo-card" data-drawer-photo="${photo.id}">${drawerPuzzleMarkup(photo)}<span><b>${drawerEscape(photo.title)}</b><small>${count>=9?'완성':`${count}/9조각`}</small></span></button>`}).join('')}</div>`}
...
function gostopErrorMessage(error){const raw=String(error?.message||error||'알 수 없는 오류'),code=String(error?.code||'');if(/PGRST202|Could not find the function|does not exist|schema cache|42P01/i.test(raw+' '+code))return `온라인 맞고·고스톱 DB 설정이 필요합니다. ZIP 안의 SUPABASE_GOSTOP_V151_UPDATE.sql을 Supabase SQL Editor에서 한 번 전체 실행해 주세요.\n[오류] ${raw}`;if(/permission denied|row-level security|42501/i.test(raw+' '+code))return `고스톱 DB 권한 오류입니다. SUPABASE_GOSTOP_V151_UPDATE.sql을 다시 실행해 주세요.\n[오류] ${raw}`;return raw}
...
function gostopSettleFinished(s,players){const code=String(s?.code||gostopRoomCode||'').toUpperCase();if(!code)return 0;const map=gostopSettlements(),old=map[code];if(old?.settled)return Number(old.amount)||0;const winner=players.find(p=>p.user_id===s.winner_id);const me=players.find(p=>p.user_id===communityUserId);if(!winner||!me)return 0;const rate=gostopRate(s.point_rate),points=Math.max(gostopThreshold(s.max_players),Number(winner.score)||0),share=points*rate;let amount=0;if(winner.user_id===communityUserId){const opponents=players.filter(p=>p.user_id!==communityUserId).length;amount=share*opponents;if(amount)stat('money',amount)}else{const loss=Math.min(Math.max(0,Number(state.stats?.money)||0),share);amount=-loss;if(loss)stat('money',-loss)}map[code]={settled:true,amount,share,points,rate,winnerId:winner.user_id,settledAt:Date.now()};saveGostopSettlements(map);addHistory(`🎴 온라인 ${Number(s.max_players)===2?'맞고':'고스톱'} · ${winner.nickname||'승자'} 승리 · ${amount>=0?'+':''}${amount.toLocaleString()}원`,`online-gostop:${code}`);save(false);render();return amount}
function gostopSettleForfeit(code,s){code=String(code||'').toUpperCase();if(!code||!s)return 0;const map=gostopSettlements(),old=map[code];if(old?.settled)return Number(old.amount)||0;const rate=gostopRate(s.point_rate),share=gostopThreshold(s.max_players)*rate,loss=Math.min(Math.max(0,Number(state.stats?.money)||0),share);if(loss)stat('money',-loss);map[code]={settled:true,amount:-loss,share,points:gostopThreshold(s.max_players),rate,forfeit:true,settledAt:Date.now()};saveGostopSettlements(map);addHistory(`🎴 온라인 ${Number(s.max_players)===2?'맞고':'고스톱'} 기권 · -${loss.toLocaleString()}원`,`online-gostop-forfeit:${code}`);save(false);render();return -loss}
...
if('serviceWorker'in navigator&&location.protocol.startsWith('http'))window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('service-worker.js?v=151-online-hwatu',{updateViaCache:'none'});await reg.update();refreshInstallUi()}catch(err){console.warn('서비스워커 등록 실패',err);refreshInstallUi()}});
