...
function closeModal(force=false){const modal=$('#modal');if(!force&&modal?.classList.contains('theme-matgo')&&window.matgoIsActive?.()){window.matgoRequestClose?.();return;}if(!force&&modal?.classList.contains('theme-online')&&gostopRoomCode){if(gostopLastStatus==='playing'&&!confirm('진행 중인 고스톱에서 나가면 게임에서 이탈합니다. 나갈까요?'))return;leaveGostopRoom(true,false);return}if(!force&&modal?.classList.contains('theme-online')&&kungRoomCode){if(kungLastStatus==='playing'&&!confirm('진행 중인 쿵쿵따에서 나가면 탈락 처리됩니다. 나갈까요?'))return;leaveKungRoom(true,false);return}if(!force&&modal?.classList.contains('theme-online')&&rummyRoomCode){if(rummyLastStatus==='playing'&&!confirm('루미큐브 대전에서 나가면 패배 처리되고 판돈을 잃습니다. 나갈까요?'))return;leaveRummyRoom(true,false,rummyLastStatus==='playing');return}if(!force&&modal?.classList.contains('theme-online')&&onlineQuizRoomCode){leaveOnlineQuizRoom(true,false);return}if(instagramLiveActive&&!force)return;if(blockingNoticeActive&&!force)return;if(memoryGameActive&&!force){if(typeof activeTrainingAbort==='function'){activeTrainingAbort();return}endMiniGameUi()}if(force&&instagramLiveActive){instagramLiveActive=false;modal.classList.remove('instagram-live-dialog');const closeButton=$('#closeModal');if(closeButton)closeButton.hidden=false}if(force&&blockingNoticeActive){blockingNoticeActive=false;const closeButton=$('#closeModal');if(closeButton)closeButton.hidden=false}if(!memoryGameActive)document.documentElement.classList.remove('minigame-active');modal.classList.remove('drawer-dialog');modal.classList.remove('ending-focus-modal');if(modal.open)modal.close();syncToastLayer();if(endingMusicMode||customEndingBgm)exitEndingMusic();if(deferredPostAdvance&&!cardRevealPending)finishDeferredPostAdvance()}
...
function openOnlineBattleHub(){
 ...
 showModal('온라인 대전',`<div class="battle-hub">
  ...
  <div class="battle-mode-grid">
   <button id="battleQuizBtn" class="battle-mode-card quiz"><span>QUIZ</span><b>류현상 퀴즈 대전</b><small>1:1 · 10문제 실시간 스피드 퀴즈</small></button>
   ...
   <button id="battleMatgoBtn" class="battle-mode-card matgo"><span>MATGO</span><b>류현상 맞고</b><small>후라보노 AI · 실제 화투판 배열 · 게임머니 점당 판돈</small></button>
   <button id="battleGostopBtn" class="battle-mode-card gostop"><span>GO-STOP</span><b>온라인 고스톱</b><small>2인 맞고 / 3인 고스톱 · 판돈 없는 점수전</small></button>
  </div>
  <p class="battle-hub-note">류현상 맞고와 루미큐브의 판돈은 게임 속 가상 보유금만 사용합니다. 온라인 고스톱은 판돈 없이 점수로만 승부합니다.</p>
 </div>`,'online');
...
function gostopErrorMessage(error){const raw=String(error?.message||error||'알 수 없는 오류'),code=String(error?.code||'');if(/PGRST202|Could not find the function|does not exist|schema cache|42P01/i.test(raw+' '+code))return `고스톱 DB 설정이 없습니다. ZIP 안의 SUPABASE_ARCADE_EXTRA_SETUP.sql을 Supabase에서 한 번 전체 실행해 주세요.\n[오류] ${raw}`;if(/permission denied|row-level security|42501/i.test(raw+' '+code))return `고스톱 DB 권한 오류입니다. SUPABASE_ARCADE_EXTRA_SETUP.sql을 다시 실행해 주세요.\n[오류] ${raw}`;return raw}
...
function gostopLobbyHtml(message=''){const nick=communityNickname(),resume=gostopStoredRoom();return `<div class="gostop-shell"><button id="gostopBackHub" class="community-back">← 대전 종목 선택</button><section class="gostop-hero"><div><small>REALTIME 2~3 PLAYERS</small><h3>고스톱</h3><p>48장 화투패로 진행하는 2인 맞고·3인 고스톱 점수전입니다. 판돈이나 현금 정산은 없습니다.</p></div><span>● LIVE</span></section>${message?`<div class="online-quiz-notice">${communityEsc(message)}</div>`:''}<section class="online-quiz-profile"><span>대전 닉네임</span><strong>${communityEsc(nick||'미설정')}</strong><button id="gostopNicknameBtn">닉네임 변경</button></section>${resume?'<button id="gostopResumeBtn" class="online-resume wide">진행 중인 고스톱 이어가기</button>':''}<section class="gostop-create"><b>새 공개방 만들기</b><label>게임 방식 <select id="gostopMaxPlayers"><option value="2">2인 맞고</option><option value="3">3인 고스톱</option></select></label><button id="gostopCreateBtn" class="primary wide" ${nick?'':'disabled'}>고스톱 방 만들기</button></section><section class="online-public-rooms"><header><div><b>대기 중인 고스톱 방</b><small>방 코드는 사용하지 않습니다.</small></div><button id="gostopRefreshLobby">새로고침</button></header><div id="gostopRoomList" class="online-room-list"><div class="online-room-loading">대기방을 불러오는 중...</div></div></section><section class="gostop-rules"><b>기본 점수</b><span>광·열끗·띠·피 자동 계산</span><span>홍단·청단·초단</span><span>고도리</span><span>점수 도달 시 GO / STOP</span><span>2인 7점 · 3인 3점부터 선택</span><span>판돈 없는 점수전</span></section></div>`}
async function openGostopLobby(message=''){gostopClearTimers();gostopRoomCode='';gostopLastStatus='';try{await gostopHealthcheck();showModal('고스톱',gostopLobbyHtml(message),'online');bindGostopLobby()}catch(error){showModal('고스톱',`<div class="online-quiz-error"><b>고스톱을 연결하지 못했습니다.</b><p>${communityEsc(gostopErrorMessage(error))}</p><button id="gostopRetry" class="primary">다시 시도</button><button id="gostopErrorBack">대전 메뉴</button></div>`,'online');$('#gostopRetry').onclick=()=>openGostopLobby();$('#gostopErrorBack').onclick=openOnlineBattleHub}}
function openGostopNickname(){const nick=communityNickname();showModal('대전 닉네임',`<div class="community-form"><input id="gostopNicknameInput" maxlength="16" value="${communityEsc(nick)}" placeholder="닉네임 입력"><div class="community-form-actions"><button id="gostopNicknameBack">취소</button><button id="gostopNicknameSave" class="primary">저장</button></div></div>`,'online');$('#gostopNicknameBack').onclick=()=>openGostopLobby();$('#gostopNicknameSave').onclick=()=>{const v=$('#gostopNicknameInput').value.trim();if(!v)return toast('닉네임을 입력해 주세요.');saveCommunityNickname(v);openGostopLobby('닉네임을 저장했습니다.')}}
function bindGostopLobby(){$('#gostopBackHub').onclick=openOnlineBattleHub;$('#gostopNicknameBtn').onclick=openGostopNickname;$('#gostopRefreshLobby').onclick=()=>refreshGostopLobby(true);$('#gostopCreateBtn').onclick=createGostopRoom;const resume=$('#gostopResumeBtn');if(resume)resume.onclick=()=>enterGostopRoom(gostopStoredRoom(),true);refreshGostopLobby();gostopLobbyTimer=setInterval(()=>refreshGostopLobby(false),2500)}
async function refreshGostopLobby(showToast=false){const target=$('#gostopRoomList');if(!target||!communityDb)return;try{const {data,error}=await communityDb.rpc('gostop_list_open_rooms');if(error)throw error;const rooms=Array.isArray(data)?data:[];if(!rooms.length){target.innerHTML='<div class="online-room-empty"><b>대기 중인 방이 없습니다.</b><span>새 방을 만들면 바로 목록에 표시됩니다.</span></div>';return}target.innerHTML=rooms.map(r=>`<article class="online-room-row"><div><b>${communityEsc(r.host_nickname||'익명')}님의 ${Number(r.max_players)===2?'맞고':'고스톱'}</b><span>${Number(r.player_count)||1}/${Number(r.max_players)||2}명</span></div><button class="primary" data-gostop-join="${communityEsc(r.room_code)}">${r.is_mine?'이어가기':'입장'}</button></article>`).join('');$$('[data-gostop-join]').forEach(b=>b.onclick=()=>joinGostopRoom(b.dataset.gostopJoin,b));if(showToast)toast('고스톱 대기방을 새로고침했습니다.')}catch(error){target.innerHTML=`<div class="online-room-empty error"><b>대기방을 불러오지 못했습니다.</b><span>${communityEsc(gostopErrorMessage(error))}</span></div>`}}
async function createGostopRoom(){if(!communityNickname())return openGostopNickname();const btn=$('#gostopCreateBtn');if(btn)btn.disabled=true;try{const max=Number($('#gostopMaxPlayers')?.value)||2;const {data,error}=await communityDb.rpc('gostop_create_room',{p_nickname:communityNickname(),p_max_players:max});if(error)throw error;await enterGostopRoom(String(data||''))}catch(error){toast(gostopErrorMessage(error));if(btn)btn.disabled=false}}
async function joinGostopRoom(code,btn=null){if(!communityNickname())return openGostopNickname();if(btn){btn.disabled=true;btn.textContent='입장 중...'}try{const {data,error}=await communityDb.rpc('gostop_join_room',{p_code:String(code||''),p_nickname:communityNickname()});if(error)throw error;await enterGostopRoom(String(data||code))}catch(error){toast(gostopErrorMessage(error));if(btn){btn.disabled=false;btn.textContent='입장'}}}
async function enterGostopRoom(code,resume=false){gostopClearTimers();gostopRoomCode=String(code||'').toUpperCase();if(!gostopRoomCode)return openGostopLobby();saveGostopStoredRoom(gostopRoomCode);showModal('고스톱','<div class="online-quiz-loading">화투판을 준비하는 중...</div>','online');try{await refreshGostopRoom();gostopPollTimer=setInterval(()=>refreshGostopRoom().catch(()=>{}),850)}catch(error){if(resume)saveGostopStoredRoom('');gostopRoomCode='';openGostopLobby(gostopErrorMessage(error))}}
...
function gostopKind(id){id=Number(id);if([0,8,28,40,44].includes(id))return'gwang';if([4,12,16,20,24,29,32,36,42,45].includes(id))return'animal';if([1,5,9,13,17,21,25,33,37,46].includes(id))return'ribbon';if([41,47].includes(id))return'doublepi';return'pi'}
...
function gostopCardHtml(id,clickable=false){return `<button class="hwatu-card ${gostopKind(id)}" ${clickable?`data-gostop-card="${id}"`: 'disabled'}><b>${gostopMonth(id)}월</b><span>${gostopKindText(id)}</span></button>`}
...
async function refreshGostopRoom(){if(!gostopRoomCode||!$('#modal')?.open)return;const s=await fetchGostopSnapshot();gostopLastStatus=s.status||'';const players=Array.isArray(s.players)?s.players:[];if(s.status==='waiting')renderGostopWaiting(s,players);else if(s.status==='playing')renderGostopPlaying(s,players);else renderGostopFinished(s,players)}
function renderGostopWaiting(s,players){const me=players.find(p=>p.user_id===communityUserId),host=s.host_id===communityUserId,active=players.filter(p=>!p.left_at),canStart=host&&active.length===Number(s.max_players)&&active.every(p=>p.ready);$('#modalTitle').textContent=Number(s.max_players)===2?'2인 맞고 · 대기실':'3인 고스톱 · 대기실';$('#modalBody').innerHTML=`<div class="gostop-shell"><button id="gostopBackLobby" class="community-back">← 공개방 목록</button><div class="gostop-player-grid">${players.map(p=>gostopPlayerHtml(p,s)).join('')}${Array.from({length:Math.max(0,Number(s.max_players)-active.length)},()=>'<div class="gostop-player empty"><b>빈 자리</b><small>참가자를 기다리는 중</small></div>').join('')}</div><div class="online-waiting-actions"><button id="gostopReadyBtn" class="${me?.ready?'ready':''}">${me?.ready?'준비 취소':'준비'}</button>${host?`<button id="gostopStartBtn" class="primary" ${canStart?'':'disabled'}>게임 시작</button>`:'<span>방장이 시작하면 게임이 시작됩니다.</span>'}<button id="gostopLeaveBtn">방 나가기</button></div></div>`;$('#gostopBackLobby').onclick=()=>leaveGostopRoom(true,true);$('#gostopLeaveBtn').onclick=()=>leaveGostopRoom(true,true);$('#gostopReadyBtn').onclick=async()=>{const {error}=await communityDb.rpc('gostop_set_ready',{p_code:gostopRoomCode,p_ready:!me?.ready});if(error)toast(gostopErrorMessage(error));else refreshGostopRoom()};const start=$('#gostopStartBtn');if(start)start.onclick=async()=>{start.disabled=true;const {error}=await communityDb.rpc('gostop_start_room',{p_code:gostopRoomCode});if(error){toast(gostopErrorMessage(error));start.disabled=false}else refreshGostopRoom()}}
function renderGostopPlaying(s,players){const mine=s.turn_user_id===communityUserId,decisionMine=s.decision_user_id===communityUserId,myHand=Array.isArray(s.my_hand)?s.my_hand:[],table=Array.isArray(s.table_cards)?s.table_cards:[],captured=Array.isArray(s.my_captured)?s.my_captured:[],turn=players.find(p=>p.user_id===s.turn_user_id),decision=players.find(p=>p.user_id===s.decision_user_id);$('#modalTitle').textContent=Number(s.max_players)===2?'2인 맞고 · 진행 중':'3인 고스톱 · 진행 중';$('#modalBody').innerHTML=`<div class="gostop-shell"><div class="gostop-player-grid compact">${players.map(p=>gostopPlayerHtml(p,s)).join('')}</div><section class="gostop-status"><b>${decision?`${communityEsc(decision.nickname)}님이 GO / STOP 선택 중`:mine?'내 차례입니다.':`${communityEsc(turn?.nickname||'상대')} 차례입니다.`}</b><span>더미 ${s.deck_count||0}장 · ${s.last_action?communityEsc(s.last_action):'게임 진행 중'}</span></section><section class="gostop-table"><header><b>바닥 패</b><small>같은 월의 패가 있으면 자동으로 한 장을 먹습니다.</small></header><div class="hwatu-grid table">${table.length?table.map(id=>gostopCardHtml(id,false)).join(''):'<span class="gostop-empty">바닥 패 없음</span>'}</div></section>${decisionMine?`<section class="gostop-decision"><b>점수가 났습니다!</b><p>계속 진행하시겠습니까?</p><button id="gostopGoBtn" class="primary">GO</button><button id="gostopStopBtn">STOP</button></section>`:''}<section class="gostop-hand"><header><b>내 손패 ${myHand.length}장</b><small>${mine&&!s.decision_user_id?'낼 패를 선택하세요.':'현재는 패를 낼 수 없습니다.'}</small></header><div class="hwatu-grid hand">${myHand.map(id=>gostopCardHtml(id,mine&&!s.decision_user_id)).join('')}</div></section><section class="gostop-captured"><header><b>내가 먹은 패</b><small>${captured.length}장</small></header><div class="hwatu-grid captured">${captured.length?captured.map(id=>gostopCardHtml(id,false)).join(''):'<span class="gostop-empty">아직 먹은 패가 없습니다.</span>'}</div></section><button id="gostopForfeitBtn">게임 나가기</button></div>`;$$('[data-gostop-card]').forEach(b=>b.onclick=()=>playGostopCard(Number(b.dataset.gostopCard),b));const go=$('#gostopGoBtn'),stop=$('#gostopStopBtn');if(go)go.onclick=()=>chooseGostop(true,go);if(stop)stop.onclick=()=>chooseGostop(false,stop);$('#gostopForfeitBtn').onclick=()=>{if(confirm('진행 중인 고스톱에서 나갈까요?'))leaveGostopRoom(true,true)}}
...
function renderGostopFinished(s,players){const winner=players.find(p=>p.user_id===s.winner_id);$('#modalTitle').textContent='고스톱 · 결과';$('#modalBody').innerHTML=`<div class="gostop-shell"><section class="gostop-result"><small>GAME OVER</small><h3>${winner?`${communityEsc(winner.nickname)} 승리!`:'게임 종료'}</h3><div class="gostop-player-grid">${players.map(p=>gostopPlayerHtml(p,s)).join('')}</div><button id="gostopResultLobby" class="primary wide">공개방 목록으로</button><button id="gostopResultHub" class="wide">대전 종목 선택</button></section></div>`;$('#gostopResultLobby').onclick=()=>leaveGostopRoom(false,true);$('#gostopResultHub').onclick=async()=>{await leaveGostopRoom(false,false);openOnlineBattleHub()}}
async function leaveGostopRoom(callServer=true,goLobby=false){const code=gostopRoomCode;gostopRoomCode='';gostopLastStatus='';saveGostopStoredRoom('');gostopClearTimers();if(callServer&&code&&communityDb){try{await communityDb.rpc('gostop_leave_room',{p_room_code:code})}catch{}}if(goLobby)return openGostopLobby();closeModal(true)}
...
if('serviceWorker'in navigator&&location.protocol.startsWith('http'))window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('service-worker.js?v=150-matgo-png',{updateViaCache:'none'});await reg.update();refreshInstallUi()}catch(err){console.warn('서비스워커 등록 실패',err);refreshInstallUi()}});
