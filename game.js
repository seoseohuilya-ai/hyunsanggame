...
// v155: online battle hub + 2~4 player Ryu Hyunsang lyric typing battle
function openOnlineBattleHub(){
 onlineQuizClearTimers();songQuizClearTimers();kungClearTimers();gostopClearTimers();
 showModal('온라인 대전',`<div class="battle-hub">
  ...
  <div class="battle-mode-grid">
   ...
   <button id="battleSongQuizBtn" class="battle-mode-card songquiz"><span>LYRIC BATTLE</span><b>류현상 노래 맞추기</b><small>2~4인 · 다음 가사 직접 입력 · 제한시간 15초</small></button>
   <button id="battleKungBtn" class="battle-mode-card kung"><span>KUNG KUNG TTA</span><b>쿵쿵따 대전</b><small>2~4인 · 3글자 끝말잇기 · 목숨 3개</small></button>
 ...
 $('#battleKungBtn').onclick=()=>openKungLobby();
...
const SONG_QUIZ_ROOM_KEY='ryuSongQuizRoomV155';
const SONG_QUIZ_PRIZE_KEY='ryuSongQuizPrizesV155';
...
function updateSongQuizLiveMeta(s,players){const score=$('#songQuizLiveScores');if(score)score.innerHTML=songQuizScoreboard(players,s);const answers=Array.isArray(s.answers)?s.answers:[],active=players.filter(p=>!p.left_at);const stateEl=$('#songQuizAnsweredState');if(stateEl)stateEl.textContent=`답변 완료 ${answers.length}/${active.length}명`;if(answers.length>=active.length&&active.length>=2&&!songQuizAdvanceLock){songQuizAdvanceLock=true;setTimeout(()=>advanceSongQuizRoom(),950)}}
async function renderSongQuizPlaying(s,players){const qres=await communityDb.rpc('songquiz_get_current_question',{p_room_code:songQuizRoomCode});if(qres.error)throw qres.error;const q=Array.isArray(qres.data)?qres.data[0]:qres.data;if(!q)return;const idx=Number(s.current_question)||0,answers=Array.isArray(s.answers)?s.answers:[],mine=answers.find(a=>a.user_id===communityUserId),active=players.filter(p=>!p.left_at),feedback=songQuizFeedback&&songQuizFeedback.question===idx?songQuizFeedback:null;songQuizRenderedQuestion=idx;songQuizTimeoutQuestion=-1;$('#modalTitle').textContent=`류현상 노래 맞추기 · ${idx+1}/${SONG_QUIZ_ROUNDS}`;const prompt=communityEsc(q.question_text||'').replace(/\n/g,'<br>');$('#modalBody').innerHTML=`<div class="songquiz-shell playing"><div id="songQuizLiveScores">${songQuizScoreboard(players,s)}</div><section class="songquiz-question"><div class="songquiz-question-top"><span>QUESTION ${idx+1} · 다음 가사를 입력하세요</span><strong id="songQuizTime">15.0</strong></div><div class="online-timebar songquiz-timebar"><i id="songQuizTimeBar" style="width:100%"></i></div><blockquote>${prompt}</blockquote><div class="songquiz-arrow">↓ 다음 가사는?</div><form id="songQuizAnswerForm" class="songquiz-answer-form"><input id="songQuizAnswer" type="text" maxlength="160" autocomplete="off" autocapitalize="off" spellcheck="false" placeholder="다음 가사를 직접 입력" ${mine?'disabled':''}><button id="songQuizSubmitBtn" class="primary" ${mine?'disabled':''}>정답 제출</button></form>${feedback?`<div class="songquiz-feedback ${feedback.correct?'correct':'wrong'}"><b>${feedback.correct?`정답! +${feedback.points}점`:'오답!'}</b><p>정답 · ${communityEsc(feedback.answer)}</p><small>${communityEsc(feedback.song)} · ${feedback.responseMs<15000?`${(feedback.responseMs/1000).toFixed(1)}초`:'시간 초과'}</small></div>`:mine?`<div class="songquiz-feedback ${mine.is_correct?'correct':'wrong'}"><b>${mine.is_correct?`정답 +${mine.points}점`:'답변 완료'}</b><p>다른 참가자의 답변을 기다리는 중...</p></div>`:''}<div id="songQuizAnsweredState" class="songquiz-answered-state">답변 완료 ${answers.length}/${active.length}명</div></section><button id="songQuizForfeitBtn" class="online-forfeit">대전 나가기</button></div>`;const form=$('#songQuizAnswerForm'),input=$('#songQuizAnswer');if(form&&!mine)form.onsubmit=e=>{e.preventDefault();submitSongQuizAnswer(idx,input?.value||'',false)};$('#songQuizForfeitBtn').onclick=()=>{if(confirm('노래 맞추기 대전에서 나갈까요? 현재 대전에서 이탈합니다.'))leaveSongQuizRoom(true,true)};startSongQuizClock(s,players);if(input&&!mine)setTimeout(()=>{try{input.focus({preventScroll:true})}catch{}},80);if(answers.length>=active.length&&active.length>=2&&!songQuizAdvanceLock){songQuizAdvanceLock=true;setTimeout(()=>advanceSongQuizRoom(),950)}}
function startSongQuizClock(s,players){if(songQuizClockTimer)clearInterval(songQuizClockTimer);const idx=Number(s.current_question)||0;const tick=()=>{if(!songQuizLastSnapshot||Number(songQuizLastSnapshot.current_question)!==idx)return;const started=new Date(songQuizLastSnapshot.question_started_at||s.question_started_at).getTime(),remain=Math.max(0,SONG_QUIZ_SECONDS-(Date.now()-started)/1000),text=$('#songQuizTime'),bar=$('#songQuizTimeBar');if(text)text.textContent=remain.toFixed(1);if(bar)bar.style.width=`${Math.max(0,Math.min(100,remain/SONG_QUIZ_SECONDS*100))}%`;const answers=Array.isArray(songQuizLastSnapshot.answers)?songQuizLastSnapshot.answers:[],active=(Array.isArray(songQuizLastSnapshot.players)?songQuizLastSnapshot.players:players).filter(p=>!p.left_at),mine=answers.find(a=>a.user_id===communityUserId);if(remain<=0&&!mine&&songQuizTimeoutQuestion!==idx&&!songQuizSubmitLock){songQuizTimeoutQuestion=idx;submitSongQuizAnswer(idx,'',true);return}if((remain<=0||answers.length>=active.length)&&active.length>=2&&!songQuizAdvanceLock){songQuizAdvanceLock=true;setTimeout(()=>advanceSongQuizRoom(),remain<=0?700:950)}};tick();songQuizClockTimer=setInterval(tick,100)}
async function submitSongQuizAnswer(questionIndex,value,timeout=false){if(songQuizSubmitLock)return;value=String(value||'').trim();if(!timeout&&!value)return toast('다음 가사를 입력해 주세요.');songQuizSubmitLock=true;const input=$('#songQuizAnswer'),btn=$('#songQuizSubmitBtn');if(input)input.disabled=true;if(btn){btn.disabled=true;btn.textContent=timeout?'시간 종료':'채점 중...'};try{const {data,error}=await communityDb.rpc('songquiz_submit_answer',{p_room_code:songQuizRoomCode,p_question_index:questionIndex,p_answer:value});if(error)throw error;const r=Array.isArray(data)?data[0]:data;songQuizFeedback={question:questionIndex,correct:!!r?.result_correct,points:Number(r?.result_points)||0,answer:String(r?.result_answer||''),song:String(r?.result_song||''),responseMs:Number(r?.result_response_ms)||15000};playSfx(songQuizFeedback.correct?'success':'fail');await refreshSongQuizRoom(true)}catch(error){if(timeout&&/이미 다음 문제|이미 답변/.test(String(error?.message||''))){await refreshSongQuizRoom(true).catch(()=>{})}else{toast(songQuizErrorMessage(error));if(input)input.disabled=false;if(btn){btn.disabled=false;btn.textContent='정답 제출'}}}finally{songQuizSubmitLock=false}}
...
// v147: 2~4인 쿵쿵따 + 2~3인 고스톱 온라인 대전
let kungRoomCode='';
let kungPollTimer=null;
let kungLobbyTimer=null;
let kungLastStatus='';
const KUNG_ROOM_KEY='ryuKungRoomV147';
function kungStoredRoom(){try{return localStorage.getItem(KUNG_ROOM_KEY)||''}catch{return ''}}
function saveKungStoredRoom(code){try{if(code)localStorage.setItem(KUNG_ROOM_KEY,code);else localStorage.removeItem(KUNG_ROOM_KEY)}catch{}}
function kungClearTimers(){if(kungPollTimer){clearInterval(kungPollTimer);kungPollTimer=null}if(kungLobbyTimer){clearInterval(kungLobbyTimer);kungLobbyTimer=null}}
function kungErrorMessage(error){const raw=String(error?.message||error||'알 수 없는 오류'),code=String(error?.code||'');if(/PGRST202|Could not find the function|does not exist|schema cache|42P01/i.test(raw+' '+code))return `쿵쿵따 DB 설정이 없습니다. ZIP 안의 SUPABASE_ARCADE_EXTRA_SETUP.sql을 Supabase에서 한 번 전체 실행해 주세요.\n[오류] ${raw}`;if(/permission denied|row-level security|42501/i.test(raw+' '+code))return `쿵쿵따 DB 권한 오류입니다. SUPABASE_ARCADE_EXTRA_SETUP.sql을 다시 실행해 주세요.\n[오류] ${raw}`;return raw}
async function kungHealthcheck(){await ensureCommunityReady();const {data,error}=await communityDb.rpc('kung_healthcheck');if(error)throw error;return data}
function kungLobbyHtml(message=''){const nick=communityNickname(),resume=kungStoredRoom();return `<div class="kung-shell"><button id="kungBackHub" class="community-back">← 대전 종목 선택</button><section class="kung-hero"><div><small>REALTIME 2~4 PLAYERS</small><h3>쿵쿵따 대전</h3><p>공개방에서 2~4명이 돌아가며 한글 3글자 단어를 잇습니다. 제한시간은 10초, 목숨은 3개입니다.</p></div><span>● LIVE</span></section>${message?`<div class="online-quiz-notice">${communityEsc(message)}</div>`:''}<section class="online-quiz-profile"><span>대전 닉네임</span><strong>${communityEsc(nick||'미설정')}</strong><button id="kungNicknameBtn">닉네임 변경</button></section>${resume?'<button id="kungResumeBtn" class="online-resume wide">진행 중인 쿵쿵따 이어가기</button>':''}<section class="kung-create"><b>새 공개방 만들기</b><label>최대 인원 <select id="kungMaxPlayers"><option value="2">2명</option><option value="3">3명</option><option value="4" selected>4명</option></select></label><button id="kungCreateBtn" class="primary wide" ${nick?'':'disabled'}>쿵쿵따 방 만들기</button></section><section class="online-public-rooms"><header><div><b>대기 중인 쿵쿵따 방</b><small>코드 없이 입장 버튼만 누르면 됩니다.</small></div><button id="kungRefreshLobby">새로고침</button></header><div id="kungRoomList" class="online-room-list"><div class="online-room-loading">대기방을 불러오는 중...</div></div></section><section class="kung-rules"><b>규칙</b><span>한글 3글자</span><span>끝 글자 이어가기</span><span>10초 제한</span><span>목숨 3개</span><span>중복 단어 금지</span><span>마지막 생존자 승리</span></section></div>`}
async function openKungLobby(message=''){kungClearTimers();kungRoomCode='';kungLastStatus='';try{await kungHealthcheck();showModal('쿵쿵따 대전',kungLobbyHtml(message),'online');bindKungLobby()}catch(error){showModal('쿵쿵따 대전',`<div class="online-quiz-error"><b>쿵쿵따 대전을 연결하지 못했습니다.</b><p>${communityEsc(kungErrorMessage(error))}</p><button id="kungRetry" class="primary">다시 시도</button><button id="kungErrorBack">대전 메뉴</button></div>`,'online');$('#kungRetry').onclick=()=>openKungLobby();$('#kungErrorBack').onclick=openOnlineBattleHub}}
function openKungNickname(){const nick=communityNickname();showModal('대전 닉네임',`<div class="community-form"><p>커뮤니티와 모든 온라인 대전에서 사용하는 닉네임입니다.</p><input id="kungNicknameInput" maxlength="16" value="${communityEsc(nick)}" placeholder="닉네임 입력"><div class="community-form-actions"><button id="kungNicknameBack">취소</button><button id="kungNicknameSave" class="primary">저장</button></div></div>`,'online');$('#kungNicknameBack').onclick=()=>openKungLobby();$('#kungNicknameSave').onclick=()=>{const v=$('#kungNicknameInput').value.trim();if(!v)return toast('닉네임을 입력해 주세요.');saveCommunityNickname(v);openKungLobby('닉네임을 저장했습니다.')}}
function bindKungLobby(){$('#kungBackHub').onclick=openOnlineBattleHub;$('#kungNicknameBtn').onclick=openKungNickname;$('#kungRefreshLobby').onclick=()=>refreshKungLobby(true);$('#kungCreateBtn').onclick=createKungRoom;const resume=$('#kungResumeBtn');if(resume)resume.onclick=()=>enterKungRoom(kungStoredRoom(),true);refreshKungLobby();kungLobbyTimer=setInterval(()=>refreshKungLobby(false),2500)}
async function refreshKungLobby(showToast=false){const target=$('#kungRoomList');if(!target||!communityDb)return;try{const {data,error}=await communityDb.rpc('kung_list_open_rooms');if(error)throw error;const rooms=Array.isArray(data)?data:[];if(!rooms.length){target.innerHTML='<div class="online-room-empty"><b>대기 중인 방이 없습니다.</b><span>새 방을 만들면 다른 플레이어에게 바로 보입니다.</span></div>';return}target.innerHTML=rooms.map(r=>`<article class="online-room-row"><div><b>${communityEsc(r.host_nickname||'익명')}님의 쿵쿵따</b><span>${Number(r.player_count)||1}/${Number(r.max_players)||4}명</span></div><button class="primary" data-kung-join="${communityEsc(r.room_code)}">${r.is_mine?'이어가기':'입장'}</button></article>`).join('');$$('[data-kung-join]').forEach(b=>b.onclick=()=>joinKungRoom(b.dataset.kungJoin,b));if(showToast)toast('쿵쿵따 대기방을 새로고침했습니다.')}catch(error){target.innerHTML=`<div class="online-room-empty error"><b>대기방을 불러오지 못했습니다.</b><span>${communityEsc(kungErrorMessage(error))}</span></div>`}}
async function createKungRoom(){if(!communityNickname())return openKungNickname();const btn=$('#kungCreateBtn');if(btn)btn.disabled=true;try{const max=Number($('#kungMaxPlayers')?.value)||4;const {data,error}=await communityDb.rpc('kung_create_room',{p_nickname:communityNickname(),p_max_players:max});if(error)throw error;await enterKungRoom(String(data||''))}catch(error){toast(kungErrorMessage(error));if(btn)btn.disabled=false}}
async function joinKungRoom(code,btn=null){if(!communityNickname())return openKungNickname();if(btn){btn.disabled=true;btn.textContent='입장 중...'}try{const {data,error}=await communityDb.rpc('kung_join_room',{p_code:String(code||''),p_nickname:communityNickname()});if(error)throw error;await enterKungRoom(String(data||code))}catch(error){toast(kungErrorMessage(error));if(btn){btn.disabled=false;btn.textContent='입장'}}}
async function enterKungRoom(code,resume=false){kungClearTimers();kungRoomCode=String(code||'').toUpperCase();if(!kungRoomCode)return openKungLobby();saveKungStoredRoom(kungRoomCode);showModal('쿵쿵따 대전','<div class="online-quiz-loading">방 정보를 불러오는 중...</div>','online');try{await refreshKungRoom();kungPollTimer=setInterval(async()=>{if(!kungRoomCode)return;try{if(kungLastStatus==='playing')await communityDb.rpc('kung_tick',{p_room_code:kungRoomCode});await refreshKungRoom()}catch{}},800)}catch(error){if(resume)saveKungStoredRoom('');kungRoomCode='';openKungLobby(kungErrorMessage(error))}}
async function fetchKungSnapshot(){const {data,error}=await communityDb.rpc('kung_get_snapshot',{p_room_code:kungRoomCode});if(error)throw error;return data||{}}
function kungPlayerHtml(p,currentSeat,winner){const me=p.user_id===communityUserId;return `<div class="kung-player ${me?'me':''} ${p.eliminated?'out':''} ${p.seat===currentSeat?'turn':''}"><b>${communityEsc(p.nickname)}</b><small>${me?'나 · ':''}${p.eliminated?'탈락':p.ready?'준비 완료':'대기'}</small><span>${p.eliminated?'💀':'❤️'.repeat(Math.max(0,Number(p.lives)||0))}</span>${winner===p.user_id?'<em>WIN</em>':''}</div>`}
function kungRemaining(started){if(!started)return 10;return Math.max(0,Math.ceil(10-(Date.now()-new Date(started).getTime())/1000))}
async function refreshKungRoom(){if(!kungRoomCode||!$('#modal')?.open)return;const s=await fetchKungSnapshot();kungLastStatus=s.status||'';const players=Array.isArray(s.players)?s.players:[];if(s.status==='waiting')renderKungWaiting(s,players);else if(s.status==='playing')renderKungPlaying(s,players);else renderKungFinished(s,players)}
function renderKungWaiting(s,players){const me=players.find(p=>p.user_id===communityUserId),host=s.host_id===communityUserId,active=players.filter(p=>!p.left_at),canStart=host&&active.length>=2&&active.every(p=>p.ready);$('#modalTitle').textContent='쿵쿵따 · 대기실';$('#modalBody').innerHTML=`<div class="kung-shell"><button id="kungBackLobby" class="community-back">← 공개방 목록</button><div class="kung-player-grid">${players.map(p=>kungPlayerHtml(p,null,null)).join('')}${Array.from({length:Math.max(0,(Number(s.max_players)||4)-active.length)},()=>'<div class="kung-player empty"><b>빈 자리</b><small>참가자를 기다리는 중</small></div>').join('')}</div><div class="online-waiting-actions"><button id="kungReadyBtn" class="${me?.ready?'ready':''}">${me?.ready?'준비 취소':'준비'}</button>${host?`<button id="kungStartBtn" class="primary" ${canStart?'':'disabled'}>게임 시작</button>`:'<span>방장이 시작하면 자동으로 시작됩니다.</span>'}<button id="kungLeaveBtn">방 나가기</button></div></div>`;$('#kungBackLobby').onclick=()=>leaveKungRoom(true,true);$('#kungLeaveBtn').onclick=()=>leaveKungRoom(true,true);$('#kungReadyBtn').onclick=async()=>{const {error}=await communityDb.rpc('kung_set_ready',{p_code:kungRoomCode,p_ready:!me?.ready});if(error)toast(kungErrorMessage(error));else refreshKungRoom()};const start=$('#kungStartBtn');if(start)start.onclick=async()=>{start.disabled=true;const {error}=await communityDb.rpc('kung_start_room',{p_code:kungRoomCode});if(error){toast(kungErrorMessage(error));start.disabled=false}else refreshKungRoom()}}
function renderKungPlaying(s,players){const turnPlayer=players.find(p=>Number(p.seat)===Number(s.current_turn_seat)),mine=turnPlayer?.user_id===communityUserId,words=Array.isArray(s.words)?s.words:[],remain=kungRemaining(s.turn_started_at),need=s.current_word?String(s.current_word).slice(-1):'자유';$('#modalTitle').textContent='쿵쿵따 · 진행 중';$('#modalBody').innerHTML=`<div class="kung-shell"><div class="kung-player-grid compact">${players.map(p=>kungPlayerHtml(p,s.current_turn_seat,null)).join('')}</div><section class="kung-board"><div class="kung-turnline"><span>${mine?'내 차례!':`${communityEsc(turnPlayer?.nickname||'상대')} 차례`}</span><strong>${remain}초</strong></div><div class="kung-current"><small>현재 단어</small><b>${communityEsc(s.current_word||'첫 단어는 자유롭게!')}</b><span>${s.current_word?`다음 글자: ${communityEsc(need)}`:'사전에 있는 3글자 단어를 입력하세요.'}</span></div><div class="kung-word-history">${words.length?words.slice().reverse().map(w=>`<span>${communityEsc(w.word)}</span>`).join(''):'<em>아직 나온 단어가 없습니다.</em>'}</div><div class="kung-input-row"><input id="kungWordInput" maxlength="3" autocomplete="off" placeholder="3글자 단어" ${mine?'':'disabled'}><button id="kungSubmitBtn" class="primary" ${mine?'':'disabled'}>제출</button></div><small class="kung-help">틀린 단어·중복·시간 초과 시 목숨이 1개 줄고 다음 사람 차례로 넘어갑니다.</small></section><button id="kungForfeitBtn">게임 나가기</button></div>`;const submit=async()=>{if(!mine)return;const input=$('#kungWordInput'),word=input?.value.trim();if(!word)return toast('3글자 단어를 입력해 주세요.');$('#kungSubmitBtn').disabled=true;const {data,error}=await communityDb.rpc('kung_submit_word',{p_room_code:kungRoomCode,p_word:word});if(error)toast(kungErrorMessage(error));else if(data&&!data.ok)toast(`${data.reason} · 목숨 ${data.lives}개`);else playSfx('success');refreshKungRoom()};$('#kungSubmitBtn').onclick=submit;const inp=$('#kungWordInput');if(inp){inp.onkeydown=e=>{if(e.key==='Enter')submit()};setTimeout(()=>inp.focus(),0)}$('#kungForfeitBtn').onclick=()=>{if(confirm('쿵쿵따에서 나가면 탈락 처리됩니다. 나갈까요?'))leaveKungRoom(true,true)}}
function renderKungFinished(s,players){const winner=players.find(p=>p.user_id===s.winner_id);$('#modalTitle').textContent='쿵쿵따 · 결과';$('#modalBody').innerHTML=`<div class="kung-shell"><section class="kung-result"><small>GAME OVER</small><h3>${winner?`${communityEsc(winner.nickname)} 승리!`:'무승부'}</h3><div class="kung-player-grid">${players.map(p=>kungPlayerHtml(p,null,s.winner_id)).join('')}</div><button id="kungResultLobby" class="primary wide">공개방 목록으로</button><button id="kungResultHub" class="wide">대전 종목 선택</button></section></div>`;$('#kungResultLobby').onclick=()=>leaveKungRoom(false,true);$('#kungResultHub').onclick=async()=>{await leaveKungRoom(false,false);openOnlineBattleHub()}}
async function leaveKungRoom(callServer=true,goLobby=false){const code=kungRoomCode;kungRoomCode='';kungLastStatus='';saveKungStoredRoom('');kungClearTimers();if(callServer&&code&&communityDb){try{await communityDb.rpc('kung_leave_room',{p_room_code:code})}catch{}}if(goLobby)return openKungLobby();closeModal(true)}
