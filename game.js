...
function closeModal(force=false){const modal=$('#modal');if(!force&&modal?.classList.contains('theme-matgo')&&window.matgoIsActive?.()){window.matgoRequestClose?.();return;}if(!force&&modal?.classList.contains('theme-online')&&gostopRoomCode){if(gostopLastStatus==='playing'&&!confirm('진행 중인 고스톱에서 나가면 게임에서 이탈합니다. 나갈까요?'))return;leaveGostopRoom(true,false,true);return}if(!force&&modal?.classList.contains('theme-online')&&kungRoomCode){if(kungLastStatus==='playing'&&!confirm('진행 중인 쿵쿵따에서 나가면 탈락 처리됩니다. 나갈까요?'))return;leaveKungRoom(true,false);return}if(!force&&modal?.classList.contains('theme-online')&&rummyRoomCode){if(rummyLastStatus==='playing'&&!confirm('루미큐브 대전에서 나가면 패배 처리되고 판돈을 잃습니다. 나갈까요?'))return;leaveRummyRoom(true,false,rummyLastStatus==='playing');return}if(!force&&modal?.classList.contains('theme-online')&&onlineQuizRoomCode){leaveOnlineQuizRoom(true,false);return}if(instagramLiveActive&&!force)return;if(blockingNoticeActive&&!force)return;if(memoryGameActive&&!force){if(typeof activeTrainingAbort==='function'){activeTrainingAbort();return}endMiniGameUi()}if(force&&instagramLiveActive){instagramLiveActive=false;modal.classList.remove('instagram-live-dialog');const closeButton=$('#closeModal');if(closeButton)closeButton.hidden=false}if(force&&blockingNoticeActive){blockingNoticeActive=false;const closeButton=$('#closeModal');if(closeButton)closeButton.hidden=false}if(!memoryGameActive)document.documentElement.classList.remove('minigame-active');modal.classList.remove('drawer-dialog');modal.classList.remove('ending-focus-modal');if(modal.open)modal.close();syncToastLayer();if(endingMusicMode||customEndingBgm)exitEndingMusic();if(deferredPostAdvance&&!cardRevealPending)finishDeferredPostAdvance()}
...
function openOnlineBattleHub(){
 onlineQuizClearTimers();rummyClearTimers();kungClearTimers();gostopClearTimers();
 showModal('온라인 대전',`<div class="battle-hub">
  ...
  <div class="battle-mode-grid">
   ...
   <button id="battleRummyBtn" class="battle-mode-card rummy"><span>RUMMY</span><b>1:1 루미큐브 대전</b><small>게임 속 보유금에서 최대 100만원까지 판돈 설정</small></button>
  ...
  <p class="battle-hub-note">후라보노AI 맞고·온라인 맞고/고스톱·루미큐브의 판돈은 모두 게임 속 가상 보유금만 사용합니다.</p>
 </div>`,'online');
...
function gostopClearTimers(){if(gostopPollTimer){clearInterval(gostopPollTimer);gostopPollTimer=null}if(gostopLobbyTimer){clearInterval(gostopLobbyTimer);gostopLobbyTimer=null}}
...
function gostopCardHtml(id,clickable=false,opts={}){const {small=false,back=false}=opts;if(back)return `<span class="matgo-card back ${small?'small':''}"><img src="assets/hwatu/back.png" alt="화투 뒷면"></span>`;const tag=clickable?'button':'span';return `<${tag} class="matgo-card online-hwatu-card ${small?'small':''} ${gostopKind(id)}" ${clickable?`data-gostop-card="${id}"`:''}><img src="${gostopCardAsset(id)}" alt="${gostopMonth(id)}월 ${gostopKindText(id)}"></${tag}>`}
...
async function leaveGostopRoom(callServer=true,goLobby=false,forfeit=false,finished=false){const code=gostopRoomCode,s=gostopLastSnapshot,status=gostopLastStatus;if(forfeit&&status==='playing')gostopSettleForfeit(code,s);gostopRoomCode='';gostopLastStatus='';gostopLastSnapshot=null;saveGostopStoredRoom('');gostopClearTimers();gostopSetWide(false);if(callServer&&code&&communityDb){try{await communityDb.rpc('gostop_leave_room',{p_room_code:code})}catch{}}if(goLobby)return openGostopLobby();closeModal(true)}
...
if('serviceWorker'in navigator&&location.protocol.startsWith('http'))window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('service-worker.js?v=154-drawer-photo-extend',{updateViaCache:'none'});await reg.update();refreshInstallUi()}catch(err){console.warn('서비스워커 등록 실패',err);refreshInstallUi()}});
