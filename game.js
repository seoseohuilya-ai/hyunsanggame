...
function dailyTick(allowRandom=true){applyDailyPracticePenalty();rollWeather();stat('hp',8);stat('stress',-4);state.items.bakcasUsedToday=0;state.items.mealsToday=0;state.items.bakcasBoughtToday=0;state.items.snacksBoughtToday=0;state.storeDaily.buskingDay=state.day;state.storeDaily.buskingCount=0;if(state.economy.lastWorkDay!==state.day-1)state.economy.workStreak=0;applySkillDecay();applyStyleDecay();updateCardCollectorQualification();if(state.day%30===0)chargeMonthlyUpkeep();return allowRandom?randomEvent():false}
...
function actionAvailability(key){
 ...
 switch(key){
  ...
  case 'album': {const cd=actionCooldownDays(state.cooldowns.album,30);reason=(state.economy?.debt||0)>0?'채무 상환 필요':state.stats.vocal<35||state.stats.compose<30?'보컬 35·작곡 30 필요':cd>0?`${cd}일 후 발매 가능`:state.stats.money<5400000?'제작비 부족 · 540만원 필요':'';break}
