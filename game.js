...
const actions={
 ...
 park:[
  ['버스킹','성공 시 보컬 +1 · 보컬·인지도·장비에 따라 수입 변화 · 시간 +1','busking'],
  ['밴드 버스킹','성공 시 보컬 +1 · 솔로보다 높은 수입 · 시간 +1','bandBusking'],
...
function stat(name,delta,rawHp=false){
 ...
 if(name==='money'){
  if(delta>0&&state.economy?.debt>0){const repay=Math.min(state.economy.debt,Math.max(1,Math.floor(delta*.5)));state.economy.debt-=repay;state.economy.totalDebtRepaid=(state.economy.totalDebtRepaid||0)+repay;state.stats.money=Math.max(0,state.stats.money+delta-repay);return delta-repay}
...
function effectiveHpCost(n){n=Math.max(0,Math.ceil(Number(n)||0));return energizerActive()?Math.max(1,Math.ceil(n/3)):n}
function costHp(n){const actual=effectiveHpCost(n);if(state.stats.hp<actual){toast(`체력이 부족합니다.${energizerActive()?` 에너자이저 적용 소모량은 ${actual}입니다.`:''}`);return false}stat('hp',-actual,true);state.lastHpCost=actual;return true}
...
function openSuspiciousShop(){
 ...
 const remain=energizerRemainingDays();
 showModal('수상한 가게',`<div class="gear-balance"><span>현재 보유금</span><strong>${state.stats.money.toLocaleString()}원</strong><small>구매에는 시간이 소모되지 않습니다.</small></div><div class="card-list"><div class="info-card mystery-shop-card"><header><b>다이어트 알약</b><span>300,000원</span></header><p>외모 -1~+1 · 체력 -10 · 스트레스 +2. 외모 +1은 매우 드물게 발생합니다.</p><button id="buyDietPill">구매</button></div><div class="info-card mystery-shop-card"><header><b>에너자이저</b><span>400,000원</span></header><p>7일 동안 행동에 필요한 모든 체력 소모량이 기존의 3분의 1로 감소합니다.${remain?` 현재 남은 효과 ${remain}일.`:''}</p><button id="buyEnergizer" ${remain?'disabled':''}>${remain?'효과 적용 중':'구매'}</button></div></div>`);
 $('#buyDietPill').onclick=buyDietPill;const e=$('#buyEnergizer');if(e)e.onclick=buyEnergizer
}
function buyDietPill(){
 if(state.stats.money<300000)return toast('다이어트 알약을 구매할 돈이 부족합니다.');
 if(state.stats.hp<effectiveHpCost(10))return toast('알약의 부작용을 버틸 체력이 부족합니다.');
 const before=snapshotStats();stat('money',-300000);costHp(10);stat('stress',2);const roll=Math.random();const lookChange=roll<.48?-1:roll<.98?0:1;if(lookChange)stat('looks',lookChange);
...
function buyEnergizer(){
 if(energizerActive())return toast(`에너자이저 효과가 ${energizerRemainingDays()}일 남았습니다.`);if(state.stats.money<400000)return toast('에너자이저를 구매할 돈이 부족합니다.');stat('money',-400000);state.effects.energizerUntilDay=state.day+6;addHistory('⚡ 수상한 가게 · 에너자이저 복용, 7일 동안 체력 소모 1/3','energizer:'+state.day);state.dialogue={name:'류현상',text:'형광빛 음료를 마시자 몸이 이상할 만큼 가벼워졌다. 앞으로 7일 동안 행동 체력 소모가 3분의 1로 감소한다.'};save(false);render();showModal('에너자이저 적용',`<div class="info-card"><b>7일 동안 체력 소모량이 3분의 1로 감소합니다.</b><p>적용 기간: ${state.day}일차부터 ${state.effects.energizerUntilDay}일차까지</p><button id="energizerConfirm" class="primary wide">확인</button></div>`);$('#energizerConfirm').onclick=openSuspiciousShop
}
...
function maybeMysteriousMerchantEvent(){
 if(state.specialEvents?.mysteriousMerchantPurchased||Math.random()>=.0018)return false;
...
function finalizeBuskingResult(ctx,rhythmResult=null){
 ...
 showDialogue('팬들',`${type} · ${weather.label} 버스킹 ${result}. 체력 ${hpCost} 소모, 팬 ${fans}명, ${money.toLocaleString()}원을 얻었다. 보컬 수입 배율 ${vocalMultiplier.toFixed(2)}배 · 인지도 Lv.${fameLv} 수입 배율 ${fameMultiplier.toFixed(2)}배 · 장비 수입 배율 ${gearMultiplier.toFixed(2)}배.${vocalGain?` 성공 경험으로 보컬 +${vocalGain}.`:''}${broken}${durabilityNotice}${rhythmLine}\n\n팬 반응: “${fanLine}”${leaveNote?`\n\n${leaveNote}`:''}\n\n${story}`);
 if(checkStalkerEvent()){if(rhythmResult&&resultBefore)setTimeout(()=>appendStatChangesToDialogue(describeActionResult(resultBefore)),260);return}advance(1);if(rhythmResult&&resultBefore)setTimeout(()=>appendStatChangesToDialogue(describeActionResult(resultBefore)),260)
}
...
function closeModal(force=false){const modal=$('#modal');if(memoryGameActive&&!force){if(typeof activeTrainingAbort==='function'){activeTrainingAbort();return}endMiniGameUi()}if(!memoryGameActive)document.documentElement.classList.remove('minigame-active');if(modal.open)modal.close();syncToastLayer();if(endingMusicMode)exitEndingMusic();if(deferredPostAdvance&&!cardRevealPending)finishDeferredPostAdvance()}
...
function render(){
 ...
 $('#scheduleList').innerHTML=`<li>현재: ${locations[state.location].name}</li><li>날씨: ${weatherLabel()} · ${dayType()}</li><li>집: ${housingInfo[state.housing][0]}</li><li>밴드 결속력: ${state.band.bond}</li><li>마이크: ${equipmentStatusText('mic')}</li><li>음향장비: ${equipmentStatusText('amp')}</li><li>보호 케이스: ${equipmentDurabilityText('battery')}</li>${energizerActive()?`<li>에너자이저: ${energizerRemainingDays()}일 남음</li>`:''}${state.manager.hired?`<li>후라보노 관계: ${state.manager.bond}</li>`:'<li>매니저: 미고용</li>'}`;
...
function openSNS(){
 ...
 showModal('SNS',`<div class="info-card"><b>팔로워·팬 ${state.stats.fans.toLocaleString()}명</b><p>팬이 많아질수록 게시물 확산력이 커져 팬·인지도 보상이 조금씩 증가합니다. 하루에 한 번 게시할 수 있습니다.</p><small>현재 팬 보상 배율 약 ${preview.fanMultiplier.toFixed(2)}배 · 인지도 배율 약 ${preview.fameMultiplier.toFixed(2)}배</small><button id="snsPost" ${canPost?'':'disabled'}>${canPost?'게시물 올리기':'오늘 게시 완료'}</button></div><div class="card-list">${snsScenarios.slice(0,6).map(x=>`<div class="info-card"><b>${x.title}</b><p>${x.text}</p></div>`).join('')}</div>`);
 const b=$('#snsPost');
 if(b)b.onclick=()=>{
  if(!canPost)return;
...
if('serviceWorker'in navigator&&location.protocol.startsWith('http'))window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('service-worker.js?v=88-suspicious-shop-collector-wedding',{updateViaCache:'none'});await reg.update();refreshInstallUi()}catch(err){console.warn('서비스워커 등록 실패',err);refreshInstallUi()}});
