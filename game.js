...
function drawerDiscoveryNotice(discovery,onDone){const count=drawerPhotoPieceCount(discovery.photo.id),x=(discovery.piece%3)*50,y=Math.floor(discovery.piece/3)*50;showBlockingNotice(discovery.complete?'사진 완성':'사진 조각 발견',`<div class="drawer-discovery-card"><div class="drawer-discovery-piece" style="--drawer-image:url('${discovery.photo.image}');--drawer-x:${x}%;--drawer-y:${y}%" aria-label="이번에 발견한 사진 조각"></div><div><b>${drawerEscape(discovery.photo.title)}</b><p>${discovery.complete?'마지막 조각을 발견해 사진이 완성되었습니다. 전체 사진은 서랍에서 확인할 수 있습니다.':`사진 한 조각을 발견했습니다. 현재 ${count}/9조각입니다.`}</p><small>전체 사진은 9조각을 모두 모은 뒤 서랍에서 공개됩니다.</small></div></div>`,onDone)}
...
function drawerPuzzleMarkup(photo,large=false){return `<div class="drawer-puzzle ${large?'large':''}" aria-label="${drawerEscape(photo.title)} 퍼즐">${state.drawer.photos[photo.id].map((found,index)=>{const x=(index%3)*50,y=Math.floor(index/3)*50;return `<div class="drawer-piece ${found?'found':'missing'}" style="--drawer-image:url('${photo.image}');--drawer-x:${x}%;--drawer-y:${y}%">${found?'':'?'}</div>`}).join('')}</div>`}
