...
const actions={
 home:[
  ['깊은 휴식','체력 +25~45 · 스트레스 -10 · 시간 +2 · 2회 연속 시 보컬·작곡 -1 · 누적 5회 이후 악몽 가능','rest'],
 ...
 store:[
  ...
  ['삼각김밥','체력 +8 · 돈 -2,500원 · 스트레스 +1 · 시간 미소모','snack'],
  ...
  ['복권','1장 시간 미소모 · 10장 자동 구매 시간 +1 · 주 100장 제한','lottery']
 ],
 practice:[
 ...
 park:[
  ['버스킹','보컬·인지도에 따라 수입 변화 · 팬·인지도 증가 · 시간 +1','busking'],
  ['밴드 버스킹','보컬·인지도에 따라 수입 변화 · 솔로보다 높은 수입 · 시간 +1','bandBusking'],
 ...
 stage:[
  ...
  ['공연','체력 -28 · 스트레스 +5 · 팬·돈·인지도 +변동 · 시간 +1','concert'],
...
function normalizeState(){
 ...
 const gambling=state.gambling||{},cards=gambling.cards||{};state.gambling={cards:{C:Math.max(0,Math.floor(Number(cards.C)||0)),U:Math.max(0,Math.floor(Number(cards.U)||0)),R:Math.max(0,Math.floor(Number(cards.R)||0)),SR:Math.max(0,Math.floor(Number(cards.SR)||0)),SEC:Math.max(0,Math.floor(Number(cards.SEC)||0)),SP:Math.max(0,Math.floor(Number(cards.SP)||0))},totalCardDraws:Math.max(0,Math.floor(Number(gambling.totalCardDraws)||0)),lotteryTickets:(Array.isArray(gambling.lotteryTickets)?gambling.lotteryTickets:[]).filter(t=>t&&Array.isArray(t.numbers)).map(t=>({id:String(t.id||`${t.purchaseDay||state.day}-${Math.random()}`),numbers:[...new Set(t.numbers.map(Number).filter(n=>n>=1&&n<=45))].slice(0,6).sort((a,b)=>a-b),purchaseDay:Math.max(1,Math.floor(Number(t.purchaseDay)||state.day)),drawDay:Math.max(1,Math.floor(Number(t.drawDay)||state.day+7)),status:t.status==='drawn'?'drawn':'pending'})).filter(t=>t.numbers.length===6),lotteryResults:(Array.isArray(gambling.lotteryResults)?gambling.lotteryResults:[]).slice(-20)};
...
function saveAudioSettings(){audioSettings.bgm=true;audioSettings.sfx=true;audioSettings.volume=Math.max(.12,Math.min(1,Number(audioSettings.volume)||.42));try{localStorage.setItem('ryuAudioSettings',JSON.stringify(audioSettings))}catch{}updateAudioButton()}
...
function forceAudioOn(){audioSettings.bgm=true;audioSettings.sfx=true;if(audioSettings.volume<.12)audioSettings.volume=.42;saveAudioSettings();const ok=ensureAudio();if(ok&&audioCtx?.state==='running'){syncAudioGains();if(!bgmTimer)startBgm()}return ok}
...
function setAudioVolume(v){audioSettings.bgm=true;audioSettings.sfx=true;audioSettings.volume=Math.max(.12,Math.min(1,Number(v)||.42));saveAudioSettings();ensureAudio();syncAudioGains()}

function gameGuideHtml(){return `<div class="game-guide">
  <section class="guide-hero">
    <span class="guide-kicker">처음 시작하는 가수를 위한 안내서</span>
    <h3>류현상을 무명 가수에서 국민가수로 성장시키세요.</h3>
  ...
  <div class="guide-pages">
    ...
    <section class="guide-page" data-guide-page="career">
      <div class="guide-callout"><b>최근 밸런스 규칙</b><p>장소 이동은 시간과 능력치를 소모하지 않지만 도착 장소의 돌발 스토리는 발생할 수 있습니다. 강제 휴식은 스트레스 50 이상·체력 15 이하에서 판정됩니다. 편의점 알바와 야간 진열 보조는 각각 누적 10·30·80회에 승급해 급여가 증가합니다. 편의점 알바 급여는 수습 45,000원→1단계 55,000원→2단계 70,000원→3단계 90,000원, 야간 진열 보조는 25,000원→32,000원→42,000원→55,000원입니다. 복권은 주 100장 제한이며 1장은 시간 미소모, 10장 구매는 시간 1칸을 소모합니다.</p></div><div class="guide-table"><div><b>훈련</b><span>보컬·작곡 성장의 기본입니다.</span></div><div><b>버스킹</b><span>초반 팬과 현금을 동시에 얻는 활동입니다.</span></div><div><b>오디션</b><span>첫 합격 보상이 크며 충분한 능력치가 필요합니다.</span></div><div><b>앨범 제작</b><span>연습실에서 진행합니다. 싱글·미니·정규 순으로 비용과 보상이 커집니다.</span></div><div><b>공연</b><span>팬, 인지도, 수익을 함께 얻습니다.</span></div><div><b>방송 출연</b><span>인지도 성장에 효과적이며 재도전 간격이 있습니다.</span></div><div><b>366일 방송 미니게임</b><span>공연장의 노래 서바이벌과 O/X 퀴즈쇼는 각각 7일 간격으로 참가합니다.</span></div><div><b>특별 이벤트</b><span>특정 날짜·레벨·조건에서 열리고 엔딩에도 영향을 줍니다.</span></div></div>
    ...
    <section class="guide-page" data-guide-page="money"><div class="guide-grid">
      <article><b>수입</b><p>아르바이트, 버스킹, 공연, 방송, 앨범으로 얻습니다. 채무가 있으면 수입 일부가 자동 상환됩니다.</p></article><article><b>고정비</b><p>월세와 생활비가 정기적으로 발생하며 매니저·밴드 유지비가 추가될 수 있습니다.</p></article><article><b>채무</b><p>비용 부족이나 고정비 미납 시 부족분이 채무가 됩니다.</p></article><article><b>채무 제한</b><p>앨범 제작, 장비·의상 구매, 이사 등 일부 투자 행동이 막힙니다.</p></article>
    </div><div class="guide-callout warning"><b>채무가 있어도 회복할 수 있습니다.</b><p>훈련, 아르바이트, 버스킹은 계속 가능합니다. 돈을 벌어 가계부에서 상환하세요.</p></div></section>
...
function toast(t){
 ...
 syncToastLayer();
 toastTimer=setTimeout(()=>{toastMessage='';syncToastLayer();toastTimer=null},3200);
}
...
function consumeBuskingEquipment(){
 ...
 if(expired.length){const message=`${expired.join('와 ')}의 내구도를 모두 사용해 장비가 사라졌습니다.`;addHistory(`🎙 장비 소모 · ${message}`,`gear-expired:${state.day}:${expired.join('-')}`);setTimeout(()=>toast(message),420);notes.push(` ${message}`)}
 return notes.join('');
}
function moveHome(){
...
function maybeRestNightmare(source='action'){
 ...
 if(data.totalRests-data.lastTriggeredRest<2)return false;
 if(Math.random()>=.40)return false;
 ...
 stat('stress',2);actionNoticeSequence++;markMajorNarrativeEvent();
 ...
 save(false);render();
 setTimeout(()=>toast('너무 오랜 휴식으로 악몽을 꿔 스트레스 받았다. · 스트레스 +2'),260);
 return true;
}
...
function openLottery(){
 let selected=[];
 const draw=()=>{const used=lotteryPurchasedThisWeek(),remain=Math.max(0,100-used),week=lotteryWeekInfo();showModal('복권 번호 선택',`<div class="info-card"><b>1~45 중 숫자 6개를 선택하세요.</b><p>1장 1,000원 · 시간 미소모 · 10장 자동 구매 10,000원 · 시간 1칸 소모</p><p>${state.day+7}일차 추첨 · 이번 주 ${week.start}~${week.end}일차 구매 <strong>${used}/100장</strong></p></div><div id="lotteryNumbers" class="lottery-number-grid">${lotteryNumberButtons(selected)}</div><p id="lotterySelectionText" class="lottery-selection">선택 ${selected.length}/6 · ${selected.length?selected.join(', '):'번호를 골라 주세요.'}</p><div class="result-actions"><button id="lotteryAuto">자동 선택</button><button id="lotteryReset">초기화</button><button id="lotteryBuy" class="primary" ${selected.length===6&&remain>=1?'':'disabled'}>선택 번호 1장 구매</button><button id="lotteryBuyTen" ${remain>=10?'':'disabled'}>10장 자동 구매</button></div><div class="info-card lottery-prize-table"><small>주간 최대 100장. 1장 구매는 시간이 지나지 않고, 10장 묶음 구매는 시간 1칸이 지나갑니다.<br>1등 6개 일치 1억원 · 2등 5개+보너스 2천만원 · 3등 5개 200만원 · 4등 4개 5만원 · 5등 3개 5천원</small></div>`);bind()};
...
function buyLotteryTickets(count=1,numbers=[]){
 ...
 addHistory(`🎟 복권 ${count}장 구매 · ${drawDay}일차 추첨 · 주간 ${used+count}/100장`,`lottery-buy:${purchaseDay}:${Date.now()}`);
 const content=count===1?`<div class="lottery-ticket"><small>추첨 예정 ${drawDay}일차 · 시간 미소모</small><h3>${tickets[0].numbers.map(n=>`<span>${n}</span>`).join('')}</h3><p>7일 뒤 자동으로 당첨 결과가 발표됩니다.</p><button id="lotteryPurchaseConfirm" class="primary wide">확인</button></div>`:`<div class="info-card"><b>복권 10장 자동 구매 완료</b><p>추첨 예정 ${drawDay}일차 · 시간 1칸 소모</p><div class="card-list">${tickets.map((t,i)=>`<small>${i+1}. ${t.numbers.join(', ')}</small>`).join('')}</div><p>이번 주 누적 ${used+count}/100장</p><button id="lotteryPurchaseConfirm" class="primary wide">확인</button></div>`;
 if(count===10){advance(1,'gambling');showModal('복권 10장 구매 완료',content)}else{pendingLocationActionStress=false;save(false);render();showModal('복권 구매 완료',content)}
 const confirm=$('#lotteryPurchaseConfirm');if(confirm)confirm.onclick=()=>closeModal()
}
function lotteryPrize(matches,bonus){if(matches===6)return {rank:'1등',money:100000000};if(matches===5&&bonus)return {rank:'2등',money:20000000};if(matches===5)return {rank:'3등',money:2000000};if(matches===4)return {rank:'4등',money:50000};if(matches===3)return {rank:'5등',money:5000};return {rank:'낙첨',money:0}}
function maybeLotteryResult(){
 ...
 const draws=new Map();
 for(const ticket of due){if(!draws.has(ticket.drawDay)){const winning=randomUniqueNumbers(6),bonus=randomUniqueNumbers(1,winning)[0];draws.set(ticket.drawDay,{winning,bonus})}}
...
function storeJobAction(key){
 ...
 toast(`${def.label} ${info.name} · ${info.reward.toLocaleString()}원 · ${progress}`);
 advance(1)
}
function doAction(key){
 ...
 buyBakcas:()=>{if(s.money<15000)return toast('돈이 부족합니다.');stat('money',-15000);state.items.bakcas++;pendingLocationActionStress=false;showDialogue('류현상',actionStory('bakcas','박칵스 하나를 가방에 넣었다. 오늘은 조금 더 버틸 수 있겠다.'));toast('박칵스 1개를 샀습니다.');save(false);render()},
 snack:()=>{if(s.hp>=100)return toast('체력이 이미 최대입니다.');if(s.money<2500)return toast('돈이 부족합니다.');stat('money',-2500);stat('hp',8);stat('stress',1);pendingLocationActionStress=false;showDialogue('류현상',`${pickActionDialogue('snack')} 삼각김밥 구매에는 시간이 지나지 않는다.`);save(false);render()},
 ...
 };pendingLocationActionStress=['store','practice','stage'].includes(state.location)&&!['wardrobe','gear','moveHome','finance','national','digimonCard','lottery','songSurvival','quizShow','buyBakcas'].includes(key);if(key!=='rest')state.restStreak=0;state.lastAction=key;if(!['wardrobe','gear','album','manager','digimonInventory','songSurvival','quizShow'].includes(key))pulseScene(key);const soundMap={stockWork:'coin',finance:'coin',flyerPromo:'tap',audienceResearch:'tap',stageRehearsal:'busking',work:'coin',buyBakcas:'coin',snack:'drink',meal:'drink',storePromo:'tap',customerPractice:'tap',digimonCard:'coin',lottery:'coin',bakcas:'drink',busking:'busking',bandBusking:'busking',concert:'busking',audition:'busking',repair:'coin'};playSfx(soundMap[key]||'tap');f[key]?.();if(!['songSurvival','quizShow'].includes(key))scheduleActionResultNotice(actionResultBefore);
}
...
function startQuizShow(){
 ...
 memoryGameActive=true;let index=0,correct=0,finished=false,answered=false;
 ...
 const finish=aborted=>{if(finished)return;finished=true;memoryGameActive=false;activeTrainingAbort=null;closeModal(true);const scored=aborted?0:correct,reward=quizRewardFor(scored);stat('money',reward.money);stat('fans',reward.fans);stat('fame',reward.fame);if(reward.stress)stat('stress',reward.stress);state.exp+=reward.exp;state.minigames.quizBest=Math.max(state.minigames.quizBest,aborted?0:correct);addHistory(`⭕ 도전 퀴즈쇼 · ${aborted?'중도 종료':`${correct}/10 정답`} · ${reward.label} · ${reward.money.toLocaleString()}원, 팬 +${reward.fans}, 인지도 +${reward.fame}`,`quiz-show:${state.day}`);state.dialogue={name:'도전 퀴즈쇼',text:aborted?'퀴즈쇼를 중도에 마쳤다. 류현상은 자신에 관한 문제인데도 왜 이렇게 긴장되는지 모르겠다며 안경을 고쳐 썼다.':correct===10?'열 문제를 모두 맞혔다. 류현상은 “내 문제니까 당연하지”라고 말했지만, 제작진은 본인도 틀리는 출연자가 꽤 많다고 알려 줬다.':`${correct}문제를 맞혔다. 틀린 문제의 해설을 듣던 류현상은 “그건 문제 표현이 애매한 것 같은데요”라며 조용히 제작진에게 항의했다.`};advance(1,'minigame');showModal('도전 퀴즈쇼 결과',`<div class="show-result-card"><span class="show-result-rank">${reward.label}</span><h3>${aborted?'중도 종료':`${correct} / 10 정답`}</h3><p>상금 <strong>${reward.money.toLocaleString()}원</strong> · 팬 <strong>+${reward.fans.toLocaleString()}</strong> · 인지도 <strong>+${reward.fame}</strong></p><small>${aborted?'중도 종료로 참가상만 지급되었습니다.':'출제 문제는 제공된 30문제에서 무작위로 나오며, 3회 동안 중복 없이 순환합니다.'}</small><button id="quizResultConfirm" class="primary wide">확인</button></div>`);$('#quizResultConfirm').onclick=()=>closeModal()};
...
function maybeCareerMilestoneEvent(){
 const lv=fameLevel();
 const defs=[
  {level:70,key:'careerLv70',title:'전국 음악 페스티벌 메인 무대',fame:300,fans:1800,money:2200000,stress:5,text:'인지도 Lv.70에 도달한 류현상은 전국 규모 음악 페스티벌의 메인 무대 제안을 받았다. 거리와 소극장에서 쌓은 경험이 처음으로 대형 야외 무대의 함성으로 이어졌다.'},
  {level:80,key:'careerLv80',title:'첫 전국 투어 매진',fame:450,fans:3000,money:4000000,stress:7,text:'인지도 Lv.80에 도달하자 첫 전국 투어가 연이어 매진됐다. 도시마다 다른 관객들이 같은 후렴을 따라 불렀고, 류현상의 이름은 더 이상 일부 음악 팬만 아는 이름이 아니게 되었다.'},
  {level:90,key:'careerLv90',title:'해외 쇼케이스 초청',fame:650,fans:5000,money:7000000,stress:8,text:'인지도 Lv.90에 도달한 류현상은 첫 해외 쇼케이스에 초청됐다. 낯선 언어의 관객이 그의 자작곡을 따라 부르며 월드 스타를 향한 마지막 구간이 열렸다.'}
 ];
 ...
 showDialogue(state.manager.hired?'후라보노':'나레이션',`${def.text}

인지도 +${def.fame}, 팬 +${def.fans.toLocaleString()}명, 수익 ${def.money.toLocaleString()}원을 얻었다.`);
 playSfx('success');save(false);render();
 const changes=describeStatChanges(before);if(changes)setTimeout(()=>toast(changes),280);
 return true;
}
function maybeMysteriousMerchantEvent(){
 if(state.specialEvents?.mysteriousMerchantPurchased||Math.random()>=.0015)return false;
...
function finalizeBuskingResult(ctx,rhythmResult=null){
 let {band,type,weather,hpCost,success,quality,fans,money,vocalMultiplier,fameMultiplier,fameLv}=ctx;
 let rhythmLine='';
 ...
 showDialogue('팬들',`${type} · ${weather.label} 버스킹 ${result}. 체력 ${hpCost} 소모, 팬 ${fans}명, ${money.toLocaleString()}원을 얻었다. 보컬 수입 배율 ${vocalMultiplier.toFixed(2)}배 · 인지도 Lv.${fameLv} 수입 배율 ${fameMultiplier.toFixed(2)}배.${broken}${durabilityNotice}${rhythmLine}\n\n팬 반응: “${fanLine}”${leaveNote?`\n\n${leaveNote}`:''}\n\n${story}`);
 if(checkStalkerEvent())return;advance(1)
}
function startBuskingRhythmGame(ctx){
 memoryGameActive=true;let finished=false,countdownCancel=null,step=0,hits=0,misses=0,timer=null,noteTimer=null;const keys=['←','↓','↑','→'];const sequence=Array.from({length:12},()=>Math.floor(Math.random()*4));
 ...
 const finish=(success,closed=false)=>{if(finished)return;finished=true;clean();memoryGameActive=false;activeTrainingAbort=null;closeModal(true);finalizeBuskingResult(ctx,{success,played:!closed,closed})};
...
function busking(band){
 ...
 const type=dayType(),weather=weatherInfo[state.weather];const baseHp=band?24:18,hpCost=baseHp+weather.hp+(type==='공휴일'?3:type==='주말'?1:0);
 if(maybeStartIziViralEvent(band,hpCost))return;if(maybeStartWaitedMoreViralEvent(band,hpCost))return;if(!costHp(hpCost))return;markSkillUse('vocal');
...
function finalizeConcertResult(ctx,dodgeResult=null){
 let {earn,newFans,concertFame}=ctx;
 let bonusText='';
 ...
 showDialogue(state.manager.hired?'후라보노':'팬들',`${state.manager.hired?`공연 수익 ${earn.toLocaleString()}원이 정산됐어요. `:''}인지도 +${concertFame}, 새 팬 ${newFans.toLocaleString()}명이 생겼습니다.${bonusText}\n\n팬 반응: “${pickFanComment(true)}”`);
 if(checkStalkerEvent())return;
 advance(1)
}
function startConcertDodgeGame(ctx){
 memoryGameActive=true;let finished=false,countdownCancel=null,moveTimer=null,spawnTimer=null,secondTimer=null;let remaining=14,lives=3,playerX=.5,spawnCount=0;const notes=[];const symbols=['♪','♫','♬','♩','𝄞'];
 ...
 const finish=(success,reason='fail',closed=false)=>{if(finished)return;finished=true;clearAll();notes.splice(0).forEach(n=>n.el?.remove());memoryGameActive=false;activeTrainingAbort=null;closeModal(true);finalizeConcertResult(ctx,{success,played:!closed,closed})};
...
function concert(){
 if(fameLevel()<15||state.stats.fans<5000||state.stats.vocal<70)return toast('공연은 인지도 Lv.15, 팬 5,000명, 보컬 70 이상이 필요합니다.');
 if(!cooldownReady('concert',7,'다음 공연'))return;if(!costHp(28))return;markSkillUse('vocal');markCooldown('concert');state.career.totalConcerts++;
 ...
 const lv=fameLevel();const concertFame=lv>=90?90:lv>=80?80:lv>=70?70:60;const ctx={earn,newFans,concertFame};
...
const specialAlbumExtras={
 ...
 careerLv70:{title:'Lv.70 · 전국 음악 페스티벌',sceneKey:'day300Promise',image:'assets/images/stage-bg.jpg',scenes:[{name:'나레이션',text:'류현상은 전국 규모 음악 페스티벌의 메인 무대에 올랐다.'},{name:'류현상',text:'공원에서 몇 사람 앞에 노래하던 시절을 떠올리며 수천 명의 관객을 향해 첫 음을 냈다.'},{name:'나레이션',text:'그날 이후 그의 이름은 지역을 넘어 전국의 공연 관계자들에게 알려졌다.'}]},
 careerLv80:{title:'Lv.80 · 전국 투어 매진',sceneKey:'day300Promise',image:'assets/images/stage-bg.jpg',scenes:[{name:'나레이션',text:'첫 전국 투어의 예매 좌석이 연이어 매진됐다.'},{name:'후라보노',text:'“형, 이제 관객이 형을 찾아서 도시를 옮겨 다녀요.”'},{name:'류현상',text:'숫자보다, 도시마다 다른 관객의 얼굴이 더 오래 기억에 남았다.'}]},
 careerLv90:{title:'Lv.90 · 해외 쇼케이스',sceneKey:'hiddenDingo',image:'assets/images/hidden-dingo-rising.png',scenes:[{name:'나레이션',text:'류현상은 첫 해외 쇼케이스 무대에 섰다. 익숙하지 않은 언어의 함성이 객석을 채웠다.'},{name:'류현상',text:'말은 달라도 노래가 시작되자 관객의 반응은 같았다.'},{name:'나레이션',text:'해외 팬들은 그의 이름과 노래를 각자의 언어로 공유하기 시작했다.'}]},
...
function describeActionResult(before){
 ...
 return parts.length?`변경 결과 · ${parts.join(' / ')}`:''
}
function scheduleActionResultNotice(before){
 const sequence=++actionNoticeSequence;
 const actionToastSerial=Number(before?.toastSerial||0);
 setTimeout(()=>{
  if(sequence!==actionNoticeSequence)return;
  // 행동 함수가 이미 성공·실패 알림을 띄웠다면 같은 결과를 다시 표시하지 않는다.
  if(toastSerial>actionToastSerial)return;
...
function checkStalkerEvent(){
 ...
 if(!state.stalker.active){state.stalker.active=true;addHistory('🚨 스토커 핵심 사건 시작 · 공연장 주변에서 설명하기 어려운 시선과 접근이 반복되기 시작했다.','stalker:start')}
 if(state.stalker.encounters>=5||state.performanceCount%6!==0)return false;
...
function checkProgress(){
 ...
 if(stalkerFatal){offerEnding('스토커 살해 엔딩','다섯 단계의 위험 신호에서 안전도 5를 확보하지 못한 채 인지도 레벨 50을 넘겼고, 통제되지 않은 접근이 돌이킬 수 없는 결말로 이어졌다.');return}
...
function showModal(title,html){const modal=$('#modal');if(modal.open)modal.close();$('#modalTitle').textContent=title;$('#modalBody').innerHTML=html;modal.showModal();syncToastLayer()}
function closeModal(force=false){const modal=$('#modal');if(memoryGameActive&&!force){if(typeof activeTrainingAbort==='function'){activeTrainingAbort();return}memoryGameActive=false}if(modal.open)modal.close();syncToastLayer();if(endingMusicMode)exitEndingMusic();if(deferredPostAdvance&&!cardRevealPending)finishDeferredPostAdvance()}
...
document.addEventListener('click',e=>{
 ...
 const allowed=target.closest?.('#choiceArea, #menuBtn, #manualSave, #backTitle, .save-manager, #modal');
 if(allowed)return;
...
if('serviceWorker'in navigator&&location.protocol.startsWith('http'))window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('service-worker.js?v=69-broadcast-minigames',{updateViaCache:'none'});await reg.update();refreshInstallUi()}catch(err){console.warn('서비스워커 등록 실패',err);refreshInstallUi()}});
